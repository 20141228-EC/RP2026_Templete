/**
 ******************************************************************************
 * @file        imu_xrobot.c
 * @author      DM_MC02
 * @brief       ATOM-IMU 成品陀螺仪 UART 通信协议层
 *
 * 本文件实现与 XRobot ATOM-IMU V5.3 成品陀螺仪的 UART 通信协议。
 *
 * ==== 通信方式说明 ====
 *
 * 通过 UART7 + DMA + 空闲中断接收数据。
 * DRV_UART_IRQHandler() 检测到 UART7 空闲中断后，
 * 调用 USART7_rxDataHandler() 进行帧解析。
 * 解析成功的数据存入全局变量 imu_data。
 *
 * ==== rx_len 计算说明 ====
 *
 * 使用 USART7_RX_BUF_LEN - DMA1_Stream7->NDTR 计算实际接收字节数。
 * drv_uart.c 的 UART7 空闲回调中：
 *   1. 先调用 USART7_rxDataHandler()
 *   2. 再重启 DMA
 *
 * ==== 与原始 UART7 回调的区别 ====
 * 1. 先调用 USART7_rxDataHandler 处理数据 
 *
 * 2. 不调用 HAL_UART_Receive_DMA（该函数受 HAL 状态机限制，
 *    ORE 溢出错误后 gState 不为 READY 会返回 HAL_ERROR），
 *    改为直接操作寄存器：
 *    __HAL_DMA_SET_COUNTER 重置 NDTR
 *    __HAL_DMA_ENABLE 重启 DMA
 *
 * 3. ORE（溢出错误）发生后，USART 硬件会自动清除 CR3 寄存器的
 *    USART_CR3_DMAR 位，导致 DMA 请求被切断。
 *    每次回调重新使能 USART_CR3_DMAR + 清除 OREF，
 *    确保 ORE 发生后 DMA 能继续工作。
 *
 * ==== 注意事项 ====
 *
 * - UART7 波特率需配置为 2000000。
 * - USART7_RX_BUF_LEN 已配置为 128（drv_uart.h），为 IMU 帧长 64 的整数倍。
 * - drv_uart.c中uart_rx_idle_callback的uart7部分需要修改
 *
 * @version V1.0
 * @date    2026-06-16
 ******************************************************************************
 */

/* Includes ------------------------------------------------------------------*/
#include "imu_xrobot.h"
#include "string.h"
#include "usart.h"
#include "drv_uart.h"

/* Exported variables --------------------------------------------------------*/
ImuUartData imu_data;

/* Private variables ---------------------------------------------------------*/
volatile uint16_t size_check;

/* Private constants ---------------------------------------------------------*/
#define IMU_FRAME_SIZE  64
#define IMU_FRAME_HEAD  0xA5
#define RAD_TO_DEG      57.29578f   /* 弧度转度 (180/pi) */

/**
 * @brief CRC8 查表
 */
static const uint8_t CRC8_TAB[256] = {
    
};

/* Private function prototypes -----------------------------------------------*/
/* Exported functions ---------------------------------------------------------*/

/**
 * @brief  CRC8 校验计算 (多项式 0x31, 初始值 0xFF)
 * @param  data  待校验数据指针
 * @param  len   数据长度
 * @return CRC8 校验值
 */
uint8_t CRC8_Calc(uint8_t *data, uint16_t len) {
    uint8_t crc = 0xff;  /* 初始值 0xFF */
    for (uint16_t i = 0; i < len; i++) {
        crc = CRC8_TAB[crc ^ data[i]];
    }
    return crc;
}

/**
 * @brief  IMU UART 初始化，清空数据缓存
 * @note   在 DEVICE_Init() 中被调用
 */
void IMU_UART_Init(void)
{
    size_check = sizeof(ImuUartData);
    memset(&imu_data, 0, sizeof(ImuUartData));
}

/* UART7 数据接收处理 ---------------------------------------------------------*/

/**
 * @brief  UART7 空闲中断数据处理器
 * @note   在 UART7 空闲中断中被 DRV_UART_IRQHandler() 调用
 *         将欧拉角从弧度转换为度
 *
 * @param  rxBuf  DMA 接收缓冲区指针
 */
void USART7_rxDataHandler(uint8_t *rxBuf)
{
    uint16_t rx_len = USART7_RX_BUF_LEN - DMA1_Stream7->NDTR;
    uint16_t i = 0;

    while (i < rx_len)
    {
        /* 查找帧头 0xA5 */
        while (i < rx_len && rxBuf[i] != IMU_FRAME_HEAD)
        {
            i++;
        }

        if (i >= rx_len)
        {
            break;
        }
        if (i + IMU_FRAME_SIZE > rx_len)
        {
            break;
        }

        uint8_t parse_buf[IMU_FRAME_SIZE];
        memcpy(parse_buf, &rxBuf[i], IMU_FRAME_SIZE);

        uint8_t calc_crc = CRC8_Calc(parse_buf, IMU_FRAME_SIZE - 1);
        uint8_t recv_crc = parse_buf[IMU_FRAME_SIZE - 1];

        if (calc_crc == recv_crc)
        {
            memcpy(&imu_data, parse_buf, sizeof(ImuUartData));

            /* 欧拉角弧度转度 */
            imu_data.eulr_.yaw *= RAD_TO_DEG;
            imu_data.eulr_.pit *= RAD_TO_DEG;
            imu_data.eulr_.rol *= RAD_TO_DEG;
        }

        i += IMU_FRAME_SIZE;
    }
}
